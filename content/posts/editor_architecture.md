...
[Ed](https://en.wikipedia.org/wiki/Ed_\(text_editor\)) (the standard Unix text
editor) were a good fit for the technical limitations of the day: the "command
...
where the beefy GPUs are.  I'm running vim over ssh (because one of my vim
plugins runs executables that need to be on the machine with the GPU), and the
...
In the machine learning community (where programming on powerful computers far
away from your laptop is common), many practitioners use notebooks like
...
save programmers from typing latency (similarly, I think that Ed's design was
mostly constrained by typewriter hardware and not round-trip-timing for
keypresses), but I do believe that development with a notebook is what made the
