...
fn generate_random_sequence(rng: &mut impl rand::Rng) -> Vec<i32> {
    let mut seq: Vec<i32> = (0..10).collect();
    seq.shuffle(rng);
    seq
}

fn compare_scores<T: Ord>(score_a: T, score_b: T) -> Outcome {
...
fn main() {
    println!("Generating 100,000 pairs of random sequences...");
    ...
    loop {
        ...
        let exact_a = score_by_exact_placement(seq_a.iter().copied());
        let exact_b = score_by_exact_placement(seq_b.iter().copied());
    ...
    println!("Saving results to heuristic_comparison_results.json...");
    let json = serde_json::to_string_pretty(&results).unwrap();
    let mut file = File::create("heuristic_comparison_results.json").unwrap();
    file.write_all(json.as_bytes()).unwrap();

    println!("Done! Results saved to heuristic_comparison_results.json");
