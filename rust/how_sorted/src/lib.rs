...
//
// A score of 0 is perfect, and larger scores are worse
pub fn score_by_num_modifications(l: im::Vector<i32>) -> Option<usize> {
    ...
    impl SearchProblem for SortProblem {
        ...
        fn heuristic(&self, node: &Self::Node) -> Self::Cost {
            score_by_exact_placement(node.iter().cloned())
        }
    ...
    let problem = SortProblem;

    match astar::astar(&problem, l) {
        Some((path, _total_cost)) => Some(path.len()),
        None => None,
    }
}
...
// A* search with a configurable heuristic function
...
pub fn true_optimal_cost(l: im::Vector<i32>) -> Option<usize> {
